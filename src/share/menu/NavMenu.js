...
import {INFOS, OPTS, OPT_STYLES} from "../../operation/func";
import "../MyLayout.css";
...
const NavMenu = ({model_names, env_names, logout, setLa, la, setEnv, env}) => {
    ...
    const getOptStyle = () => {
        let opt = "";
        let style = "";
        if (OPT_STYLES.includes(url[url.length - 1])) {                     // OptResult, ModelRecord, ModelFactor
    ...
    const getModelUrl = (model) => {
        if (OPTS.includes(opt) && OPT_STYLES.includes(style)) {     // OptResult, OptRecord, OptFactor
    ...
    const getOptUrl = (toOpt) => {
        if (url.length === 3 && model_names.includes(url[1]) && OPTS.includes(opt)) {   // OptParam
            ...
            navigate(`/${url[1]}/${toOpt}`)
        } else if (url.length === 4 && model_names.includes(url[1]) && OPTS.includes(opt) && OPT_STYLES.includes(style)) {
